...
// Expose reinit functions for data-loader.js
...
window.reinitSkillBars = function() {
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.querySelectorAll('.skill-fill').forEach(fill => {
          const w = fill.dataset.width;
          if (w) { fill.style.width = '0'; requestAnimationFrame(() => { fill.style.transition = 'width 1s ease'; fill.style.width = w + '%'; }); }
        });
        observer.unobserve(e.target);
      }
    });
  }, { threshold: 0.2 });
  document.querySelectorAll('.skill-cat-card').forEach(card => observer.observe(card));
};
